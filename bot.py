...
s = db.read() or '{}' #читаем содержимое файла
_songs = json.loads(s) #если получилось прочитать, то загружаем. если нет - обнуляем
...
def forward_song(msg):
    ...
    bot.send_audio(_admin, msg.audio.file_id, caption=f'[prislal](tg://user?id={msg.from_user.id})', parse_mode='Markdown') #в личку с ссылкой на аккаунт
...
def start():
    try:
        ...
        bot.send_message(_admin, f'Ooops... [{e}]') #дергаем админа
